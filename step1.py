...
class BatchRemotePipeline:
    def __init__(self):
        self.base_dir = os.path.expanduser("~/student/students_webserver/jiajun/run")
...
使用示例:
  ...
  python batch_remote_pipeline.py --results-only  # 只提取已有结果
    ...
    parser.add_argument('--input_json', '-i', type=str,
                        help='仅处理一个指定的JSON文件名（如 Q8PUQ1.json）')

    args = parser.parse_args()

    pipeline = BatchRemotePipeline()
    if args.input_json:
        json_file = args.input_json.strip()
        if not json_file.endswith('.json'):
            json_file += '.json'

        full_path = os.path.join(pipeline.input_dir, json_file)

        if not os.path.exists(full_path):
            print(f"❌ 指定的 JSON 文件不存在: {full_path}")
            sys.exit(1)

        print(f"🔬 仅处理指定文件: {json_file}")
        success = pipeline.process_single_sequence(json_file)
